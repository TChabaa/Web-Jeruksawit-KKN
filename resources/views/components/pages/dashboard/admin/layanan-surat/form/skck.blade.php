        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg">
            <form action="{{ route(auth()->user()->role . '.layanan-surat.submit', $type) }}" method="POST"
                ...
                <div class="px-6 py-4 border-t border-gray-200 dark:border-gray-600">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-white">Detail SKCK</h3>
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-300">Informasi khusus untuk permohonan SKCK</p>
                </div>
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="keperluan" id="keperluan" value="{{ old('keperluan') }}"
                                required placeholder="Contoh: Melamar kerja, daftar kuliah, dll"
                                class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                            @error('keperluan')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <label for="tanggal_mulai_berlaku" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Tanggal
                                Mulai Berlaku *</label>
                            <input type="date" name="tanggal_mulai_berlaku" id="tanggal_mulai_berlaku"
                                value="{{ old('tanggal_mulai_berlaku') }}" required
                                class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                        ...
                        <div>
                            <label for="tanggal_akhir_berlaku" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Tanggal
                                Akhir Berlaku *</label>
                            <input type="date" name="tanggal_akhir_berlaku" id="tanggal_akhir_berlaku"
                                value="{{ old('tanggal_akhir_berlaku') }}" required
                                class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                // Set minimum date for validity dates to today
                const today = new Date().toISOString().split('T')[0];
                document.getElementById('tanggal_mulai_berlaku').min = today;
                document.getElementById('tanggal_akhir_berlaku').min = today;

                // Update end date minimum when start date changes
                document.getElementById('tanggal_mulai_berlaku').addEventListener('change', function() {
                    document.getElementById('tanggal_akhir_berlaku').min = this.value;
                });
            });
        </script>
    @endpush
