                ...
                <div class="px-6 pb-4">
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            <label for="alamat_tujuan" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Alamat Tujuan
                                Pindah *</label>
                            <textarea name="alamat_tujuan" id="alamat_tujuan" rows="3" required
                                placeholder="Masukkan alamat lengkap tujuan pindah termasuk RT/RW, Kelurahan/Desa, Kecamatan, Kota/Kabupaten, Provinsi"
                                class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white">{{ old('alamat_tujuan') }}</textarea>
                            @error('alamat_tujuan')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <!-- Alasan Pindah -->
                        <div class="sm:col-span-2">
                            <label for="alasan_pindah" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Alasan Pindah
                                *</label>
                            <input type="text" name="alasan_pindah" id="alasan_pindah"
                                value="{{ old('alasan_pindah') }}" required
                                placeholder="Contoh: Pekerjaan, Pendidikan, Mengikuti Keluarga, dll"
                                class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                            @error('alasan_pindah')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <!-- Tanggal Pindah -->
                        <div>
                            <label for="tanggal_pindah" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Tanggal
                                Rencana Pindah *</label>
                            <input type="date" name="tanggal_pindah" id="tanggal_pindah"
                                value="{{ old('tanggal_pindah') }}" required
                                class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                    ...
                    <div class="grid grid-cols-1 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="purpose" id="purpose" value="{{ old('purpose') }}"
                                required placeholder="Contoh: Pengurusan pindah domisili, administrasi kependudukan"
                                class="mt-1 block w-full border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                            @error('purpose')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            <label for="notes" class="block text-sm font-medium text-gray-700 dark:text-gray-200">Catatan
                                Tambahan</label>
                            <textarea name="notes" id="notes" rows="3" placeholder="Informasi tambahan terkait kepindahan"
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                // Set minimum date for tanggal_pindah to today
                const today = new Date().toISOString().split('T')[0];
                document.getElementById('tanggal_pindah').min = today;
            });
        </script>
    @endpush
</x-layouts.dashboard>
